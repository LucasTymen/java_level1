...
This means we can access each sub-array in the 2D array using a loop!
...
Previously, we learned that we can use the double brackets [][],
where the first set of brackets contains the index of the element of the outer array and the second set of brackets contains the index of the element in the subarray.
...
You might be wondering how we can figure out the number of iterations needed in order to fully traverse the 2D array.
    ...
    In order to find the number of elements in the subarray, we can get the length of the subarray after it has been retrieved from the outer array.
        ...
        In most cases, getting the length of the first subarray in the 2D array will apply to the rest of the subarrays
        (if it is rectangular in shape), but there are rare occasions where the length of the subarrays could be different.
        This occurs if the 2D array is a jagged array. We won’t be working with any jagged 2D arrays in this lesson, but it’s something to keep in mind.
...
Store the number of subarrays of intMatrix into an int variable called numSubArrays.
...
Use twoDArray.length to get the number of subarrays.
Checkpoint 2 Step instruction is unavailable until previous steps are completed

2.

Store the length of the subarrays using the first subarray in intMatrix. Store it in a variable called subArrayLength. This should be an int.
...
Also, add a line of code which calculates the sum of all of the elements in the 2D array.
Concept Review
