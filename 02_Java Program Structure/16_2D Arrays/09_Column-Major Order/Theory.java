...
Column-major order has the same starting and finishing point as row-major order, but it’s traversal is completely different
...
==> https://www.codecademy.com/resources/docs/java/strings
*/
...
Because of these changes to our for loops, our iterator a now iterates through every column while our iterator b iterates through every row.
==> https://www.codecademy.com/resources/docs/java/iterator

Since our iterators now represent the opposite values, whenever we access an element from our 2D array, we need to keep in mind what indices we are passing to our accessor.
Remember the format we use for accessing the elements matrix[row][column]?
...
We will be using the same runner data from the last exercise, but this time we are going to take the average times per lap rather than per runner.
This requires that we use column-major traversal.
Instructions
Checkpoint 1 Enabled

1.

You are provided with some runner lap data. Take a look at the loops we’re using to iterate through this 2D array.
Replace the incorrect for loop headers to perform column-major traversal.
