...
==> https://www.codecademy.com/resources/docs/java/calendar
...
==> https://www.codecademy.com/resources/docs/java/comments
...
We need to know which cells to look at (skipping column titles for example), which cells to ignore (empty data, invalid data, outliers, etc.),
and which cells to convert (converting string input from a file to numbers).

Let’s try an example!

We are making a simple grayscale image editor program and we want to apply some modifications to the image.
We have a 4x8 pixel image that is stored as a 2D array of integers.
...
Now that you have your empty image, use nested for loops to copy over the data from the original image (stored in imageData) to the new image,
