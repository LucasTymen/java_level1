...
/*
Here we’re creating an instance of a Dog and using that Dog to call the speak() method.
...
/*
Take a look at the resetSettings() method in particular.
This method calls other methods from the class.
But it needs an object to call those methods!
Rather than create a new object (like we did with the Dog named myDog earlier), we use this as the object.
...
If you’re writing another method of the Computer, and want to call the pairWithOtherComputer() method, you could use this as the parameter.
...
We’ve given you a Person class with three instance variables — age, wisdom, and fitness.
Those values all get initialized to different values when the constructor is called.
We’ve also provided you with mutator methods for all three variables.

Complete the hasBirthday() method.
This method should call the mutator methods to increase age by 1, increase wisdom by 5, and decrease fitness by 3.

When you’re done writing hasBirthday() look at the main() method to see how we’re calling it.

Within the hasBirthday() method, you should be calling three methods using the this keyword — this.setAge(), this.setWisdom() and this.setFitness().

Each of those methods take a parameter.
The parameter for setAge() should be this.age + 1 — we’re setting the new age to be the current age plus one.
Concept Review
