...
Take a look at the main() method of Bank.java.
Right now it’s trying to directly access and change the balance in a checking account.
Unfortunately, that instance variable is private.
Run your code with Bank.java open.
...
getBalance() should return an int.
Then, in Bank.java, replace the reference to bankOfGods.accountOne.balance with bankOfGods.accountOne.getBalance().
3.

Let’s do the same for a mutator method.
In CheckingAccount.java, write a mutator method named setBalance() that has an int parameter named newBalance.
The method should set the balance instance variable to the value passed into the method.
