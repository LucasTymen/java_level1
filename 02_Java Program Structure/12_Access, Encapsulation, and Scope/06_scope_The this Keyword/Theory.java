...
Throughout the rest of this lesson, we’ll use this. when referring to an instance variable.
...
Take a look at the instance variables for our SavingsAccount class.
There are three of them. owner, balanceDollar, balanceEuro.
Next, take a look at the constructor we’ve started for you.
The job of the constructor is to initialize these instance variables.

Complete the constructor to give value to all three instance variables.
The instance variable owner should be set to the local variable owner.
The instance variable balanceDollar should be set equal to the local variable balanceDollar.
Finally, the instance variable balanceEuro should be set equal to the local variable balanceDollar multiplied by 0.85.
(Right now, every dollar is worth 0.85 euros.)

You can set the instance variable owner equal to the local variable owner by writing this.owner = owner; inside the constructor.
2.

Next, look at the addMoney() method.
This method should add the value stored in the parameter balanceDollar to the instance variable balanceDollar.
Before adding the money to the account, print out the statement "Adding ____ dollars to the account.".
After adding the money, print the statement "The new balance is _____ dollars.".
Be careful of spacing, capitalization, and punctuation with your print statements.

Call the addMoney() method once in the main() method to test your code. Add 2000 dollars to Zeus’s account.

(Note that this function should probably be updating balanceEuro as well based on the parameter.
We won’t test your code for that, but you can try implementing it yourself.
Also, note that balanceDollar is a pretty bad name for the parameter of this function.
Something like moneyToAdd would make more sense.
But for the sake of practicing local variables vs instance variables, keep the parameter named balanceDollar.)

The first print statement should be System.out.println("Adding " + balanceDollar + " dollars to the account.")

When you update the balance, you should do this.balanceDollar += balanceDollar.

Finally, when writing the second print statement, make sure to print the instance variable, using this.balanceDollar.
