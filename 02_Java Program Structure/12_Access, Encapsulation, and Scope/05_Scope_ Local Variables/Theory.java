...
The same is true for parameters. The scope of those parameters is only the method they’re associated with.
If you try to use a parameter outside the function it’s defined in, you’ll get an error. T
hese variables are often called local variables.
...
==> https://www.codecademy.com/resources/docs/java/conditionals
==> https://www.codecademy.com/resources/docs/java/loops

If you declare a variable inside the body of a conditional or in a loop, that variable can only be used inside that structure.
This also includes the variable you’re using as your looping variable.
...
If a variable is defined inside curly braces, and you try to use that variable outside of those curly braces, you will likely see an error!
Instructions
1.

Take a look at the main() method which tries to sum the values from a list.
Right now, we’re looping through the values and trying to store the sum in a variable called sum.
However, that variable is declared inside the for loop.

There are two problems with this.
The first is a logical error.
Since we’re setting int sum = 0; at the start of the loop, every time the loop runs, sum gets reset back to 0.

However, there is also a syntactical error.
Right now we try to print sum after the loop.
But that’s outside of sum‘s scope since the variable was declared inside the loop.

Run your code as-is. You should expect to get an error related to the scope of sum.
2.

Move the line of code int sum = 0; to be above the for loop.
Now the for loop has access to sum and we’re able to return sum after the loop finishes.
